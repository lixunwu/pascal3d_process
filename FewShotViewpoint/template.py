...
cats = ['aeroplane', 'bicycle', 'boat', 'bottle', 'bus', 'car', 'chair', 'diningtable', 'motorbike', 'sofa', 'train',
        'tvmonitor']
...
if not os.path.exists(annotation_file):
    with open(annotation_file, 'a') as f:
        f.write(
            'source,set,cat,im_name,object,cad_index,truncated,occluded,difficult,azimuth,elevation,inplane_rotation,left,upper,right,lower,has_keypoints,distance,im_path\n')
    ...
    for database in databases:
        ...
        for cat in cats:
            ...
            if database == 'imagenet':
                ...
                val_list_file = os.path.join('PASCAL', 'VOCdevkit', 'VOC2012', 'ImageSets', 'Main',
                                             '{}_val.txt'.format(cat))
            ...
            for n in trange(len(im_files),
                            desc='reading category {} for {} set of {} database'.format(cat, subset, database)):
                im_file = im_files[n]
                ...
                for i in range(objects.shape[0]):
                    ...
                    with open(annotation_file, 'a') as f:
                        f.write(
                            database + ',' + subset + ',' + cat + ',' + im_name + ',' + str(i) + ',' + cad_index + ',' +
                            truncated + ',' + occluded + ',' + difficult + ',' + azimuth + ',' + elevation + ',' + inplane_rotation + ',' +
                            left + ',' + upper + ',' + right + ',' + lower + ',' + has_keypoint + ',' + distance + ',' + os.path.join(
                                image_dir, im_file) + '\n')
